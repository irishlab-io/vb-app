...
_USER = (2, "alice", "pass", "1111111111", 800.0,
         False, None, None, "Hello", False)
_USER2 = (3, "bob", "pass2", "2222222222", 500.0,
          False, None, None, None, False)
_ADMIN = (1, "admin", "adminpass", "0000000001", 9999.0,
          True, None, None, None, False)
_SUSPENDED = (4, "eve", "pass", "3333333333", 100.0,
...
class TestRegisterLoginDashboardFlow:
    ...
    def test_register_success_then_login_returns_token(self, client):
        ...
        db_user = (5, "newuser", "pass123", "9876543210", 1000.0,
                   False, None, None, None, False)
    ...
    def test_suspended_user_cannot_login_or_reach_dashboard(self, client):
        ...
        with patch("vuln_bank.app.execute_query",
                   return_value=[_SUSPENDED]):
            resp = client.post("/login",
                               json={"username": "eve", "password": "pass"})
...
class TestTransferFlow:
    ...
    def test_successful_transfer_reduces_balance(self, client,
                                                  user_auth_headers):
    ...
    def test_transfer_then_history_shows_entry(self, client,
                                                user_auth_headers):
        sender_data = ("1111111111", 1000.0)
        mock_txn = (1, "1111111111", "2222222222", 250.0,
                    "2024-01-01 10:00:00", "transfer", "Rent")
    ...
    def test_transfer_blocked_when_insufficient_funds(self, client,
                                                       user_auth_headers):
    ...
    def test_transfer_requires_auth_then_succeeds_with_token(self, client,
                                                               user_token):
        # First attempt without token → 401
        resp_no_auth = client.post("/transfer",
                                   json={"to_account": "2222222222",
                                         "amount": 10.0})
...
class TestPasswordResetFlow:
    ...
    def test_forgot_password_exposes_pin_then_reset_succeeds(self, client):
        ...
        with patch("vuln_bank.app.execute_query",
                   side_effect=[[(2,)], None]):
            fp_resp = client.post("/forgot-password",
                                  json={"username": "alice"})
        ...
        with patch("vuln_bank.app.execute_query",
                   side_effect=[[(2,)], None]):
            reset_resp = client.post("/reset-password",
                                     json={"username": "alice",
    ...
    def test_api_v1_forgot_password_exposes_pin(self, client):
        with patch("vuln_bank.app.execute_query",
                   side_effect=[[(2,)], None]):
            resp = client.post("/api/v1/forgot-password",
                               json={"username": "alice"})
        assert resp.status_code == 200
    ...
    def test_api_v2_forgot_password_hides_pin(self, client):
        with patch("vuln_bank.app.execute_query",
                   side_effect=[[(2,)], None]):
            resp = client.post("/api/v2/forgot-password",
                               json={"username": "alice"})
        assert resp.status_code == 200
        debug = resp.get_json()["debug_info"]
    ...
    def test_api_v3_forgot_password_generates_4_digit_pin(self, client):
        ...
        with patch("vuln_bank.app.execute_query",
                   side_effect=[[(2,)], None]):
            with patch("vuln_bank.app.random.randint",
                       side_effect=mock_randint):
                resp = client.post("/api/v3/forgot-password",
                                   json={"username": "alice"})
        assert resp.status_code == 200
...
class TestVirtualCardFlow:
    ...
    def test_create_card_then_list_cards(self, client, user_auth_headers):
        ...
        with patch("vuln_bank.app.execute_query",
            ...
            create_resp = client.post("/api/virtual-cards/create",
                                      headers=user_auth_headers,
                                      json={"card_limit": 500.0,
                                            "card_type": "standard",
                                            "currency": "USD"})
        ...
        # Step 2: List cards
        mock_card = (42, "1234567890123456", "123", "04/25",
                     500.0, 0.0, False, True,
                     "2024-01-01 00:00:00", None, "standard", "USD")
        with patch("vuln_bank.app.execute_query",
                   return_value=[mock_card]):
            list_resp = client.get("/api/virtual-cards",
                                   headers=user_auth_headers)
    ...
    def test_toggle_freeze_changes_card_state(self, client, user_auth_headers):
        ...
        with patch("vuln_bank.app.execute_query",
                   return_value=[(True,)]):
            resp = client.post("/api/virtual-cards/42/toggle-freeze",
                               headers=user_auth_headers)
        assert resp.status_code == 200
        ...
        with patch("vuln_bank.app.execute_query",
                   return_value=[(False,)]):
            resp2 = client.post("/api/virtual-cards/42/toggle-freeze",
                                headers=user_auth_headers)
        assert "unfrozen" in resp2.get_json()["message"].lower()

    def test_fund_card_deducts_from_main_balance(self, client,
                                                   user_auth_headers):
        user_row = ("1111111111", 1000.0)
        # (id, user_id, card_number, card_limit, current_balance,
        #  is_frozen, currency, card_type)
        card_row = (42, 2, "1234567890123456", 500.0, 0.0,
                    False, "USD", "standard")

        with patch("vuln_bank.app.execute_query",
                   side_effect=[[user_row], [card_row]]):
            with patch("vuln_bank.app.execute_transaction"):
                resp = client.post("/api/virtual-cards/42/fund",
                                   headers=user_auth_headers,
                                   json={"amount": 100.0})
        assert resp.status_code == 200
        data = resp.get_json()
    ...
    def test_fund_frozen_card_is_rejected(self, client, user_auth_headers):
        user_row = ("1111111111", 1000.0)
        card_row_frozen = (42, 2, "1234567890123456", 500.0, 0.0,
                           True, "USD", "standard")
        with patch("vuln_bank.app.execute_query",
                   side_effect=[[user_row], [card_row_frozen]]):
            resp = client.post("/api/virtual-cards/42/fund",
                               headers=user_auth_headers,
                               json={"amount": 100.0})
        assert resp.status_code == 400

    def test_fund_card_insufficient_main_balance(self, client,
                                                   user_auth_headers):
    ...
    def test_card_requires_auth(self, client):
        assert client.post("/api/virtual-cards/create",
                           json={}).status_code == 401
        assert client.get("/api/virtual-cards").status_code == 401
        assert client.post(
            "/api/virtual-cards/1/toggle-freeze").status_code == 401

    def test_card_transactions_listed(self, client, user_auth_headers):
        mock_txn = (1, 42, 25.0, "Amazon", "purchase",
                    "completed", "2024-01-01 10:00:00",
                    "Online shopping", "1234567890123456", "USD")
        with patch("vuln_bank.app.execute_query",
                   return_value=[mock_txn]):
            resp = client.get("/api/virtual-cards/42/transactions",
                              headers=user_auth_headers)
        assert resp.status_code == 200
...
class TestBillPaymentFlow:
    ...
    def test_list_billers_by_category(self, client):
        mock_billers = [
            (1, 1, "PowerCo", "BILLER001", "Electric", 10.0, 1000.0, True)
        ]
        with patch("vuln_bank.app.execute_query",
                   return_value=mock_billers):
            resp = client.get("/api/billers/by-category/1")
        assert resp.status_code == 200
        billers = resp.get_json()["billers"]
        assert billers[0]["name"] == "PowerCo"

    def test_bill_payment_balance_method_success(self, client,
                                                   user_auth_headers):
        ...
        with patch("vuln_bank.app.execute_query",
            ...
            with patch("vuln_bank.app.execute_transaction"):
                resp = client.post("/api/bill-payments/create",
                                   headers=user_auth_headers,
                                   json={"biller_id": 1,
                                         "amount": 50.0,
    ...
    def test_bill_payment_insufficient_balance(self, client,
                                                user_auth_headers):
        payer = (2, "alice", "1111111111", 10.0)
        biller = ("BILLER001", "PowerCo", "Utilities")

        with patch("vuln_bank.app.execute_query",
                   side_effect=[[payer], [biller]]):
            resp = client.post("/api/bill-payments/create",
                               headers=user_auth_headers,
                               json={"biller_id": 1,
                                     "amount": 500.0,
    ...
    def test_bill_payment_requires_auth(self, client):
        resp = client.post("/api/bill-payments/create",
                           json={"biller_id": 1, "amount": 50.0,
                                 "payment_method": "balance"})
        assert resp.status_code == 401

    def test_bill_payment_via_frozen_card_rejected(self, client,
                                                    user_auth_headers):
        payer = (2, "alice", "1111111111", 800.0)
        biller = ("BILLER001", "PowerCo", "Utilities")
...
class TestAdminLoanApprovalFlow:
    ...
    def test_user_requests_loan_then_admin_approves(self, client,
                                                     user_auth_headers,
                                                     admin_auth_headers):
        # Step 1: Regular user requests a loan
        with patch("vuln_bank.app.execute_query"):
            req_resp = client.post("/request_loan",
                                   headers=user_auth_headers,
                                   json={"amount": 500.0})
        ...
        with patch("vuln_bank.app.execute_query",
                   return_value=[loan_row]):
            with patch("vuln_bank.app.execute_transaction"):
                approve_resp = client.post("/admin/approve_loan/1",
    ...
    def test_approve_nonexistent_loan_returns_404(self, client,
                                                   admin_auth_headers):
        with patch("vuln_bank.app.execute_query", return_value=[None]):
            resp = client.post("/admin/approve_loan/999",
                               headers=admin_auth_headers)
        assert resp.status_code == 404
...
class TestAdminCreateAdminFlow:
    def test_admin_creates_new_admin_account(self, client,
                                              admin_auth_headers):
        with patch("vuln_bank.app.execute_query"):
            resp = client.post("/admin/create_admin",
                               headers=admin_auth_headers,
...
class TestApiAuthRoutes:
    ...
    def test_api_login_success(self, client):
        ...
        mock_cursor.fetchone.return_value = (
            2, "alice", "pass", "1111111111", 800.0, False,
...
class TestRateLimitingFlow:
    ...
    def test_unauthenticated_rate_limit_blocks_after_limit(self, client):
        ...
        with patch("vuln_bank.app.ai_agent", ai_mock):
            ...
            for _ in range(UNAUTHENTICATED_LIMIT):
                resp = client.post("/api/ai/chat/anonymous",
                                   json={"message": "hi"},
                                   environ_overrides=_EXTERNAL_IP)
                assert resp.status_code == 200

            # Next request should be blocked
            blocked = client.post("/api/ai/chat/anonymous",
                                  json={"message": "hi"},
                                  environ_overrides=_EXTERNAL_IP)
    ...
    def test_different_ips_have_independent_limits(self, client):
        ...
        with patch("vuln_bank.app.ai_agent", ai_mock):
            ...
            for _ in range(UNAUTHENTICATED_LIMIT):
                client.post("/api/ai/chat/anonymous",
                            json={"message": "hi"},
                            environ_overrides=ip_a)

            # IP B should still be under its own limit
            resp_b = client.post("/api/ai/chat/anonymous",
                                 json={"message": "hi"},
                                 environ_overrides=ip_b)
        assert resp_b.status_code == 200

    def test_rate_limit_status_endpoint_accessible(self, client):
        resp = client.get("/api/ai/rate-limit-status",
                          environ_overrides=_EXTERNAL_IP)
        assert resp.status_code == 200
...
class TestIdorFlow:
    ...
    def test_user_not_found_returns_404(self, client, user_auth_headers):
        with patch("vuln_bank.app.execute_query", return_value=[]):
            resp = client.get("/api/v3/user/9999",
                              headers=user_auth_headers)
        assert resp.status_code == 404
...
class TestApiTransactionsFlow:
    def test_returns_transactions_for_account(self, client,
                                               user_auth_headers):
        mock_txn = (1, "1111111111", "2222222222", 100.0,
                    "2024-01-01 10:00:00", "transfer", "Test")
        with patch("vuln_bank.app.execute_query",
                   return_value=[mock_txn]):
    ...
    def test_missing_account_number_returns_400(self, client,
                                                 user_auth_headers):
...
class TestProfilePictureFlow:
    ...
    def test_upload_no_file_returns_400(self, client, user_auth_headers):
        resp = client.post("/upload_profile_picture",
                           headers=user_auth_headers,
                           data={},
                           content_type="multipart/form-data")
        assert resp.status_code == 400
