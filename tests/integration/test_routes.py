...
class TestLoginRoute:
    ...
    def test_suspended_account_returns_403(self, client):
        with patch("vuln_bank.app.execute_query", return_value=[_SUSPENDED]):
            resp = client.post(
                "/login", json={"username": "suspended", "password": "pass"}
            )
...
class TestRegisterRoute:
    def test_successful_registration(self, client):
        ...
        with patch("vuln_bank.app.execute_query", side_effect=[[], [new_user]]):
            resp = client.post(
    ...
    def test_duplicate_username_returns_400(self, client):
        with patch("vuln_bank.app.execute_query", return_value=[("existinguser",)]):
            resp = client.post(
                "/register", json={"username": "existinguser", "password": "pass123"}
            )
...
class TestCheckBalanceRoute:
    def test_known_account_returns_balance(self, client):
        with patch("vuln_bank.app.execute_query", return_value=[("testuser", 500.0)]):
...
class TestTransferRoute:
    def test_requires_authentication(self, client):
        resp = client.post(
            "/transfer", json={"to_account": "9876543210", "amount": 100.0}
        )
        assert resp.status_code == 401

    def test_transfer_with_sufficient_funds(self, client, user_auth_headers):
        with patch(
            "vuln_bank.app.execute_query", return_value=[("1234567890", 1000.0)]
        ):
    ...
    def test_transfer_with_insufficient_funds(self, client, user_auth_headers):
        with patch("vuln_bank.app.execute_query", return_value=[("1234567890", 50.0)]):
            resp = client.post(
                "/transfer",
                headers=user_auth_headers,
                json={"to_account": "9876543210", "amount": 100.0},
    ...
    def test_transfer_returns_new_balance(self, client, user_auth_headers):
        with patch("vuln_bank.app.execute_query", return_value=[("1234567890", 500.0)]):
            with patch("vuln_bank.app.execute_transaction"):
                resp = client.post(
                    "/transfer",
                    headers=user_auth_headers,
                    json={"to_account": "9876543210", "amount": 200.0},
                )
...
class TestUpdateBioRoute:
    ...
    def test_bio_update_success(self, client, user_auth_headers):
        with patch("vuln_bank.app.execute_query"):
            resp = client.post(
                "/update_bio", headers=user_auth_headers, json={"bio": "My new bio"}
            )
        assert resp.status_code == 200
        data = resp.get_json()
        assert data["status"] == "success"
    ...
    def test_bio_echoes_input(self, client, user_auth_headers):
        ...
        with patch("vuln_bank.app.execute_query"):
            resp = client.post(
                "/update_bio", headers=user_auth_headers, json={"bio": payload}
            )
...
class TestGraphQLRoute:
    ...
    def test_post_without_auth_returns_401(self, client):
        resp = client.post(
            "/graphql", json={"query": "{ transactionSummary { scope } }"}
        )
        assert resp.status_code == 401

    def test_post_without_query_returns_400(self, client, user_auth_headers):
        resp = client.post("/graphql", headers=user_auth_headers, json={})
        assert resp.status_code == 400
...
class TestAdminRoutes:
    ...
    def test_admin_panel_denies_non_admin(self, client, user_auth_headers):
        assert (
            client.get("/sup3r_s3cr3t_admin", headers=user_auth_headers).status_code
            == 403
        )

    def test_approve_loan_denies_non_admin(self, client, user_auth_headers):
        assert (
            client.post("/admin/approve_loan/1", headers=user_auth_headers).status_code
            == 403
        )
    ...
    def test_delete_account_denies_non_admin(self, client, user_auth_headers):
        assert (
            client.post(
                "/admin/delete_account/2", headers=user_auth_headers
            ).status_code
            == 403
        )

    def test_toggle_suspension_denies_non_admin(self, client, user_auth_headers):
        assert (
            client.post(
                "/admin/toggle_suspension/2", headers=user_auth_headers
            ).status_code
            == 403
        )

    def test_admin_panel_accessible_for_admin(self, client, admin_auth_headers):
        with patch(
            ...
            resp = client.get("/sup3r_s3cr3t_admin", headers=admin_auth_headers)
        assert resp.status_code == 200

    def test_approve_loan_succeeds_for_admin(self, client, admin_auth_headers):
        mock_loan = (1, 2, 500.0, "pending")
        with patch("vuln_bank.app.execute_query", return_value=[mock_loan]):
            with patch("vuln_bank.app.execute_transaction"):
                resp = client.post("/admin/approve_loan/1", headers=admin_auth_headers)
        assert resp.status_code == 200
        assert resp.get_json()["status"] == "success"

    def test_delete_account_succeeds_for_admin(self, client, admin_auth_headers):
        with patch("vuln_bank.app.execute_query"):
            resp = client.post("/admin/delete_account/3", headers=admin_auth_headers)
        assert resp.status_code == 200

    def test_toggle_suspension_succeeds_for_admin(self, client, admin_auth_headers):
        mock_target = (3, "targetuser", False, False)
        with patch("vuln_bank.app.execute_query", side_effect=[[mock_target], None]):
            resp = client.post("/admin/toggle_suspension/3", headers=admin_auth_headers)
        assert resp.status_code == 200
    ...
    def test_cannot_suspend_own_account(self, client, admin_auth_headers):
        ...
        resp = client.post("/admin/toggle_suspension/1", headers=admin_auth_headers)
        assert resp.status_code == 400

    def test_toggle_suspension_user_not_found(self, client, admin_auth_headers):
        with patch("vuln_bank.app.execute_query", return_value=[]):
            resp = client.post(
                "/admin/toggle_suspension/999", headers=admin_auth_headers
            )
        assert resp.status_code == 404
...
class TestInternalRoutes:
    def test_internal_secret_blocked_externally(self, client):
        resp = client.get("/internal/secret", environ_overrides=_EXTERNAL_IP)
        assert resp.status_code == 403

    def test_internal_config_blocked_externally(self, client):
        resp = client.get("/internal/config.json", environ_overrides=_EXTERNAL_IP)
        assert resp.status_code == 403

    def test_metadata_root_blocked_externally(self, client):
        resp = client.get("/latest/meta-data/", environ_overrides=_EXTERNAL_IP)
        assert resp.status_code == 403

    def test_metadata_ami_blocked_externally(self, client):
        resp = client.get("/latest/meta-data/ami-id", environ_overrides=_EXTERNAL_IP)
        assert resp.status_code == 403
