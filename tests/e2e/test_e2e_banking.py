...
class TestNewUserBankingJourney:
    ...
    def test_register_login_balance_transfer_history(self, e2e_client):
        ...
        txn_row = (1, "9000000001", "9000000002", 200.0,
                   "2024-06-01 12:00:00", "transfer", "E2E payment")
        with patch("vuln_bank.app.execute_query", return_value=[txn_row]):
            hist = e2e_client.get(
                "/transactions/9000000001", headers=headers
            )
    ...
    def test_profile_picture_upload_then_dashboard_shows_picture(
        ...
        img_data = BytesIO(b"GIF89a\x01\x00\x01\x00\x00\xff\x00,\x00\x00"
...
class TestVirtualCardLifecycle:
    ...
    _CARD_ACTIVE = (1, "4111111111111111", "123", "12/26",
                    500.0, 0.0, False, True,
                    "2024-01-01", None, "standard", "USD")
    _CARD_FUNDED = (1, "4111111111111111", "123", "12/26",
                    500.0, 200.0, False, True,
                    "2024-01-01", None, "standard", "USD")
    ...
    def test_create_then_fund_then_freeze_then_unfreeze(
        ...
        # Create
        with patch("vuln_bank.app.execute_query",
                   return_value=[("9000000001", 5000.0)]):
            with patch("vuln_bank.app.execute_transaction"):
        ...
        with patch("vuln_bank.app.execute_query",
                   return_value=[self._CARD_FUNDED]):
            cards = e2e_client.get(
                "/api/virtual-cards", headers=user_auth_headers
            )
    ...
    def test_card_transactions_visible_after_funding(
        ...
        txn_row = (1, 1, 50.0, "merchant_x", "purchase",
                   "completed", "2024-06-01 12:00:00",
                   "Test payment", "4111111111111111", "USD")
        with patch("vuln_bank.app.execute_query",
                   return_value=[txn_row]):
            resp = e2e_client.get(
...
class TestBillPaymentJourney:
    ...
    _PAYMENT = (1, 2, 1, 200.0, "balance", None, "REF-0001",
                "success", "2024-06-01 12:00:00", None,
                "Bill Payment", "City Power", "Utilities", None)

    def test_browse_categories_without_auth(self, e2e_client):
        """Bill categories are publicly accessible (no token required)."""
        with patch("vuln_bank.app.execute_query",
                   return_value=[self._CATEGORY]):
    ...
    def test_browse_billers_by_category(self, e2e_client):
        ...
        _full_biller = (1, 1, "City Power", "CITY_POWER_ACC",
                        "Electric utility", 10.0, 10000.0)
        with patch("vuln_bank.app.execute_query",
                   return_value=[_full_biller]):
            resp = e2e_client.get("/api/billers/by-category/1")
        assert resp.status_code == 200
        assert resp.get_json()["billers"][0]["name"] == "City Power"

    def test_pay_bill_then_check_history(
        self, e2e_client, user_auth_headers
    ):
...
class TestLoanRequestToApprovalJourney:
    ...
    def test_admin_approves_loan_requested_by_user(
        ...
        with patch("vuln_bank.app.execute_query",
                   side_effect=[[self._LOAN_ROW], [account_row]]):
            with patch("vuln_bank.app.execute_transaction"):
